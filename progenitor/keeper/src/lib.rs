...
pub mod types {
    ...
    /// (within the account) public key. One account may have large number of\n
    /// access keys. Access keys allow to act on behalf of the account by
    ...
    /// used to replace account-level public keys.",
    ///      "type": "string",
    ///      "enum": [
    ///        "FullAccess"
    ///      ]
    ...
    pub enum Action {
        ...
        /// here <http://nomicon.io/Primitives/Account.html>.
    ...
    /// a shard would like to send to another shard.\n When a shard wants to
    /// send a lot of receipts to another shard, it needs to create a request
    ...
    /// This allows nodes to push and pull chunks from a central stream.\n The
    /// two benefits of this approach are: (1) less request/response traffic\n
    /// on the peer-to-peer network and (2) lower latency for RPC nodes indexing
    /// the chain.",
    ///  "type": "object",
    ///  "required": [
    ...
    ///    },
    ///    "min_tx_gas": {
    ///      "description": "The minimum amount of gas in a chunk spent on
    /// converting new transactions\n to receipts, as long as the receiving
    /// shard is not congested.\n\n See [`CongestionControlConfig`] for more
    /// details.",
    ///      "type": "integer",
    ///      "format": "uint64",
    ///      "minimum": 0.0
    ...
    /// shard in the `shards_produced` array.",
    ///      "default": [],
    ///      "type": "array",
    ///      "items": {
    ///        "type": "integer",
    ...
    /// corresponds to the shard in the `shards_endorsed` array.",
    ///      "default": [],
    ///      "type": "array",
    ///      "items": {
    ///        "type": "integer",
    ...
    /// flag in production, because on both mainnet\n and testnet it would be
    ...
    /// number of concurrent requests\n to reduce the performance impact of
    /// state sync.",
    ///      "default": 5,
    ///      "type": "integer",
    ///      "format": "uint32",
    ...
    /// to deserialize old data, use execution error for new data",
    ///      "type": "object",
    ///      "required": [
    ///        "LinkError"
    ///      ],
    ...
    /// key to pay for function call gas and\n transaction fees. When this
    /// access key is used, both account balance and the allowance is\n
    ...
    /// not a limiter itself, but it's a value we use for
    /// initial_memory_pages.",
    ///      "type": "integer",
    ///      "format": "uint32",
    ///      "minimum": 0.0
    ...
    /// RPCs without influencing the\n protocol thus changing it per-node
    ...
    /// until\n some limit is reached. This time limit ensures that adding
    /// transactions won't take\n longer than the specified duration, which
    /// helps to produce the chunk quickly.",
    ///      "type": "string"
    ///    },
    ///    "produce_empty_blocks": {
    ...
    /// database in DBCol::LatestChunkStateWitnesses.\n Saving the latest
    /// witnesses is useful for analysis and debugging.\n When this option is
    /// enabled, the node will save ALL witnesses it observes, even invalid
    /// ones,\n which can cause extra load on the database. This option is not
    /// recommended for production use,\n as a large number of incoming
    /// witnesses could cause denial of service.",
    ///      "type": "boolean"
    ///    },
    ///    "save_trie_changes": {
    ...
    pub struct RpcClientConfigResponse {
        ...
        /// witnesses is useful for analysis and debugging.
        /// When this option is enabled, the node will save ALL witnesses it
        /// observes, even invalid ones, which can cause extra load on
        /// the database. This option is not recommended for production use,
        /// as a large number of incoming witnesses could cause denial of
        /// service.
    ...
    pub enum RpcTransactionStatusRequest {
        ...
        Variant1 {
            ...
            tx_hash: CryptoHash,
            #[serde(default, skip_serializing_if = "::std::option::Option::is_none")]
            wait_until: ::std::option::Option<TxExecutionStatus>,
        },
    }
    ...
    /// shards -- by calculate the hash of account\n id and mod number of
    /// shards. This is added to capture the old `account_id_to_shard_id`
    ...
    /// boundaries between shards.\n Each shard contains a range of accounts
    /// from one boundary account to\n another - or the smallest or largest
    /// account possible. The total\n number of shards is equal to the number of
    /// boundary accounts plus 1.",
    ///      "type": "array",
    ///      "items": {
    ///        "$ref": "#/components/schemas/AccountId"
    ///      }
    ...
    /// shard\n Since shard_ids always range from 0 to num_shards - 1, we use
    ...
    /// is unreliable and for debugging purposes only. It's also probably ok
    ...
    ///    },
    ///    {
    ///      "description": "Indicates that a resharding operation on flat storage is already in progress,\n when it wasn't expected to be so.",
    ///      "type": "string",
    ///      "enum": [
